...
\begin{titlepage}
  \centering
  \vspace*{3cm}
  {\LARGE\bfseries FusionGuardNet: Audio Deepfake Detection\\[0.5em]
  Using WavLM, Whisper, and Nes2Net\par}
  \vspace{2cm}
  {\large Roy Naor\par}
  \vspace{0.4cm}
...
0.60\%}, demonstrating that performance remains stable under a broader and more
diverse set of spoofing conditions.
...
from scratch, we leverage large pre-trained models \cite{chen2022wavlm,radford2023whisper}
that have already learned rich speech representations from massive audio data.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={rectangle, rounded corners=4pt, draw=gray!60, fill=NavyBlue!8,
                text width=3.3cm, align=center, minimum height=0.85cm, inner sep=5pt},
    gbox/.style={rectangle, rounded corners=4pt, draw=OliveGreen!60, fill=OliveGreen!15,
                text width=3.3cm, align=center, minimum height=0.85cm, inner sep=5pt,
                font=\small\bfseries},
    fbox/.style={rectangle, rounded corners=4pt, draw=Goldenrod!80, fill=Goldenrod!18,
                text width=3.3cm, align=center, minimum height=0.85cm, inner sep=5pt},
    arr/.style={-{Stealth[length=4pt]}, thick, gray!65}
  ]
...
\begin{equation}
  \mathbf{f}_t = \sigma_1 \odot \mathbf{w}_t \;+\; \sigma_2 \odot \mathbf{s}_t,
  \qquad [\boldsymbol{\sigma}_1, \boldsymbol{\sigma}_2]
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{lrX}
    ...
    Nes2Net input            & $200\times768$        & No projection layer needed \\
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{lX}
    ...
    EER                & Threshold-independent metric: operating point where
...
Liu, X., et al. (2023).
\textit{ASVspoof 2021: Towards spoofed and deepfake speech detection in the
wild}.
...
\textit{Harder or different? Understanding generalization of audio deepfake
detection}.
...
\textit{SLIM: Style-linguistics mismatch model for generalized audio deepfake
detection}.
...
\textit{WavLM: Large-scale self-supervised pre-training for full stack speech
processing}.
...
\textit{Comprehensive layer-wise analysis of SSL models for audio deepfake
detection}.
...
\textit{Audio deepfake detection with self-supervised WavLM and multi-fusion
...
\textit{wav2vec~2.0: A framework for self-supervised learning of speech
representations}.
...
\& Kwak, I.-Y. (2024).
\textit{Experimental study: Enhancing voice spoofing detection models with
wav2vec~2.0}. arXiv:2402.17127.
