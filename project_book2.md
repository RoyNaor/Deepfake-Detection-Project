...
The motivation behind this design is that synthetic speech can fail in more than one way. Some artifacts are acoustic, while others are phonetic. Combining two models that analyze the signal differently gives the system a better chance of detecting both [16][17]. If one representation is less sensitive to a specific type of fake audio, the other representation may still contain useful information that helps the classifier reach the correct decision.
